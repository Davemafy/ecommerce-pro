...
export function ProductDetailPage() {
  ...
  const history=useMemo(()=>[...productOrders].sort((a,b)=>new Date(b.date).getTime()-new Date(a.date).getTime()).map(order=>({date:order.date,action:'Order fulfilled',quantity:-order.quantity,user:'CommercePro',notes:`#${order.id}`})),[productOrders]);
  ...
  return <main className="figma-page product-detail-page">
    ...
    <div className="product-bento">
      ...
      <div className="product-side-stack">
        ...
        <section className="revenue-card"><span>TOTAL REVENUE</span><strong>${productRevenue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><p>{productOrders.length} recorded order{productOrders.length===1?'':'s'}</p></section>
      </div>
    </div>
    ...
    <section className="card sales-card">
      ...
      <div className="sales-chart-area"><div className="y-axis"><span>${maxChart.toFixed(0)}</span><span>${(maxChart*.66).toFixed(0)}</span><span>${(maxChart*.33).toFixed(0)}</span><span>$0</span></div><div className="bars">{chart.map((item,index)=><i key={`${range}-${item.label}-${index}`} style={{height:`${Math.max(4,(item.value/maxChart)*100)}%`,opacity:item.value?0.45+index*.08:.12}} title={`${item.label}: $${item.value.toFixed(2)}`}/>)}</div></div>
      <div className="x-axis">{chart.filter((_,index)=>index===0||index===Math.floor(chart.length/2)||index===chart.length-1).map((item)=><span key={item.label}>{item.label}</span>)}</div>
    </section>
    ...
    <Modal open={editing} title="Edit product" description="Update the catalog details and stock quantity." onClose={()=>{setEditing(false);setError('')}} footer={<><button onClick={()=>{setEditing(false);setError('')}}>Cancel</button><button className="primary" onClick={save}>Save changes</button></>}>
      <FormFields values={form} onChange={(name,value)=>{setError('');setForm((current)=>({...current,[name]:value}))}} fields={[{name:'name',label:'Product name',required:true},{name:'category',label:'Category',required:true},{name:'price',label:'Price',type:'number',required:true},{name:'stock',label:'Stock quantity',type:'number',required:true}]}/>{error&&<p className="form-error">{error}</p>}
    </Modal>
