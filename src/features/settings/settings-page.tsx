...
export function SettingsPage(){
  ...
  const chooseLogo=(file?:File)=>{
    ...
    if(file.size>5*1024*1024){toast('Choose an image smaller than 5 MB','error');return;}
    const reader=new FileReader();
    reader.onload=()=>{
      const value=typeof reader.result==='string'?reader.result:'';
      setLogoPreview(value||null);
      setForm((current:any)=>({...current,logoName:file.name,logoData:value}));
    };
    reader.onerror=()=>toast('Could not read that image','error');
    reader.readAsDataURL(file);
  };
  const removeLogo=()=>{setLogoPreview(null);setForm((current:any)=>({...current,logoName:'',logoData:''}));};

  return <main className="figma-page settings-figma">
    <div className="tabs" role="tablist" aria-label="Settings sections">{tabs.map(t=><button key={t} role="tab" aria-selected={active===t} className={active===t?'active':''} onClick={()=>navigate(`/settings/${t}`)}>{labels[t]}</button>)}</div>

    {active==='general'?<>
      <div className="settings-general-grid">
        <div className="settings-main">
          <section className="card settings-section"><h2>Store Details</h2><label>Store Name<input value={form.storeName||''} onChange={e=>setForm((x:any)=>({...x,storeName:e.target.value}))}/></label><label>Store Logo</label><div className="logo-setting"><div className={`logo-placeholder${logoPreview?' has-preview':''}`}>{logoPreview?<img src={logoPreview} alt="Store logo preview"/>:<ImageIcon aria-hidden="true"/>}</div><div><div className="logo-actions"><button onClick={()=>logoInputRef.current?.click()}>Change</button><button className="danger-link" disabled={!logoPreview&&!form.logoName} onClick={removeLogo}>Remove</button></div><input ref={logoInputRef} type="file" accept="image/png,image/jpeg,image/gif,image/svg+xml" hidden onChange={e=>chooseLogo(e.target.files?.[0])}/><p>{form.logoName||'SVG, PNG, JPG or GIF (max. 5 MB)'}</p></div></div></section>
          <section className="card settings-section"><h2>Regional Settings</h2><div className="regional-grid"><label>Default Currency<select value={form.currency||'USD'} onChange={e=>setForm((x:any)=>({...x,currency:e.target.value}))}><option value="USD">USD ($) - US Dollar</option><option value="NGN">NGN (₦) - Nigerian Naira</option><option value="GBP">GBP (£) - British Pound</option><option value="EUR">EUR (€) - Euro</option></select></label><label>Store Timezone<select value={form.timezone||'Pacific Time (PT) - US & Canada'} onChange={e=>setForm((x:any)=>({...x,timezone:e.target.value}))}><option>Pacific Time (PT) - US & Canada</option><option>West Africa Time (WAT) - UTC+1</option><option>Greenwich Mean Time (GMT)</option><option>Eastern Time (ET) - US & Canada</option></select></label></div></section>
        ...
        <aside className="card settings-help"><h3><Info aria-hidden="true"/>About General Settings</h3><p>These settings define your store's primary identity and how it handles regional data. The store name appears on receipts and customer emails.</p><small>Saved frontend preferences persist between browser sessions.</small></aside>
      </div>
      <section className="settings-danger-zone"><h2>Danger Zone</h2><div className="settings-danger-card"><div><b>Delete Store</b><p>Once you delete a store, there is no going back. Please be certain.</p></div><button className="danger-button" onClick={()=>setDeleteOpen(true)}>Delete Store</button></div></section>
    </>:<section className="card settings-panel"><header><h2>{labels[active]}</h2><p>{sectionCopy[active]}</p></header><div className="settings-panel-body"><SettingsFields section={active} form={form} setForm={setForm}/></div><footer><button onClick={reset}>Cancel</button><button className="primary" onClick={save}>Save Changes</button></footer></section>}

    <ConfirmDialog open={deleteOpen} title="Delete this store?" description="This destructive action is disabled in the frontend preview so demo data is protected." confirmLabel="Confirm deletion" danger onClose={()=>setDeleteOpen(false)} onConfirm={()=>{setDeleteOpen(false);toast('Store deletion is disabled in this preview','error')}}/>
  </main>;
}
